...
-- Compatibility shim for TBC Anniversary C_Container API changes
if C_Container then
...
-- HasKey() compatibility - doesn't exist in TBC Anniversary, keyring is always available in TBC
if not HasKey then
	HasKey = function() return true end
end

-- LE_ITEM_CLASS_CONTAINER compatibility
if not LE_ITEM_CLASS_CONTAINER then
	LE_ITEM_CLASS_CONTAINER = Enum.ItemClass and Enum.ItemClass.Container or 1
end

-- ContainerIDToInventoryID compatibility (moved to C_Container)
if not ContainerIDToInventoryID then
	if C_Container and C_Container.ContainerIDToInventoryID then
		ContainerIDToInventoryID = C_Container.ContainerIDToInventoryID
	else
		-- Fallback: manual calculation (bag slots start at inventory slot 20)
