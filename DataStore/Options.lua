...
function addon:SetupOptions()
	if DataStoreGeneralOptions then
		addon:AddOptionCategory(DataStoreGeneralOptions, addonName)
	end
	pcall(function() LibStub("LibAboutPanel").new(addonName, addonName) end)
	if DataStoreHelp then
		addon:AddOptionCategory(DataStoreHelp, HELP_LABEL, addonName)
	end

	if DataStoreHelp_Text then
		addon:SetupInfoPanel(help, DataStoreHelp_Text)
	end
	
	if DataStoreGeneralOptions_Title then
	...
	local width = InterfaceOptionsFramePanelContainer and InterfaceOptionsFramePanelContainer:GetWidth() or 600
	width = width - 45
	if DataStoreHelp then DataStoreHelp:SetWidth(width) end
	if DataStoreHelp_ScrollFrame then DataStoreHelp_ScrollFrame:SetWidth(width) end
	if DataStoreHelp_Text then DataStoreHelp_Text:SetWidth(width-35) end
end
...
function addon:OnUpdate(self, mandatoryResize)
	...
	if DataStoreHelp then DataStoreHelp:SetWidth(OptionsPanelWidth-45) end
	if DataStoreHelp_ScrollFrame then DataStoreHelp_ScrollFrame:SetWidth(OptionsPanelWidth-45) end
	if DataStoreHelp then DataStoreHelp:SetHeight(OptionsPanelHeight-30) end
	if DataStoreHelp_ScrollFrame then DataStoreHelp_ScrollFrame:SetHeight(OptionsPanelHeight-30) end
	if DataStoreHelp_Text then DataStoreHelp_Text:SetWidth(OptionsPanelWidth-80) end
end
