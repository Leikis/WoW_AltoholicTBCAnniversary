...
-- TBC Anniversary API Compatibility Shims
if C_AddOns then
...
-- GetItemSubClassInfo compatibility (moved to C_Item namespace)
if C_Item and C_Item.GetItemSubClassInfo and not GetItemSubClassInfo then
...
if not LE_ITEM_CLASS_TRADEGOODS then
	LE_ITEM_CLASS_TRADEGOODS = Enum.ItemClass and Enum.ItemClass.Tradegoods or 7
end
if not LE_ITEM_CLASS_CONTAINER then
	LE_ITEM_CLASS_CONTAINER = Enum.ItemClass and Enum.ItemClass.Container or 1
end

-- InterfaceOptions_AddCategory compatibility (removed in 10.0, use Settings API)
...
function addon:OnInitialize()
	...
	-- Frame is now created in Frames.xml to ensure it exists before child module XML files load
	addon.Frames = DataStoreFrames or CreateFrame("Frame", "DataStoreFrames", UIParent)
