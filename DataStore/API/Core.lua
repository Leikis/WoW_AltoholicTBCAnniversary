...
function addon:RegisterModule(moduleName, module, publicMethods, allowOverrides)
	...
	local charKey = GetKey()
	-- Ensure the character entry exists (AceDB wildcards should create it, but force it if needed)
	if not db.Characters[charKey] then
		db.Characters[charKey] = {}
	end
	module.ThisCharacter = db.Characters[charKey]
